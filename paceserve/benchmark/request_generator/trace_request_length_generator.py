...
from paceserve.benchmark.config import TraceRequestLengthGeneratorConfig
from paceserve.benchmark.request_generator.base_request_length_generator import (
...
class TraceRequestLengthGenerator(BaseRequestLengthGenerator):
    ...
    def get_next_num_tokens(self) -> Tuple[float, float]:
        if self.next_request_idx >= len(self.trace_df):
            self.trace_df = self.trace_df.sample(frac=1)
            self.next_request_idx = 0
        ...
        return (
            ...
            row["num_decode_tokens"] if not self.config.num_decode_override else self.config.num_decode_override,
        )
