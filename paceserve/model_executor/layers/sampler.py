...
from paceserve.core.datatypes.sampling_params import SamplingType
from paceserve.core.datatypes.sequence import (
    SamplerOutput,
    SamplerOutputs,
    SequenceMetadata,
)
from paceserve.model_executor.parallel_utils.tensor_parallel import (
...
class Sampler(nn.Module):
    ...
    def __init__(self, embedding: torch.Tensor, vocab_size: int, use_logits: int) -> None:
        super().__init__()
    ...
    def forward(
        ...
        if self.use_logits:
            seq_logits = self.compute_log_probs_for_sequence(hidden_states, seq_metadata_list)
        ...
        if self.use_logits:
            return [
                SamplerOutput(seq_metadata_list[i].seq.seq_id, flashinfer_sample_result[i], seq_logits[i])
    ...
    def compute_log_probs_for_sequence(
        ...
        """
        Compute log-probabilities over the vocabulary for each position in the sequence.

        Returns:
            log_probs: Tensor of shape [seq_len, vocab_size]
        """
        seq_logits = []
        start = 0
        for i in range(len(seq_metadata_list)):
            seq_metadata = seq_metadata_list[i]
            prompt_len = seq_metadata.prompt_chunk_len
            seq_hidden = hidden_states[start: start+prompt_len]
            logits = torch.matmul(seq_hidden, self.embedding.t())  # [seq_len, vocab_size]
            logits = gather_from_tensor_model_parallel_region(logits)
            logits = logits[:, :self.vocab_size]
            log_probs = torch.nn.functional.log_softmax(logits, dim=-1)
            start += prompt_len
            seq_logits.append(log_probs)
        return seq_logits
