...
from paceserve.config import MetricsConfig, ModelConfig, ReplicaConfig
...
from paceserve.core.datatypes.sequence import Sequence, SequenceMetadata
from paceserve.metrics.cdf_sketch import CDFSketch
from paceserve.metrics.constants import (
...
from paceserve.metrics.data_series import DataSeries
from paceserve.core.datatypes.scheduler_metadata import SchedulerMetadata
from paceserve.metrics.tracer import Tracer
...
class MetricsStore:
    ...
    def __init__(
        ...
        self.id = replica_config.replica_id
    ...
    def reset(self):
        ...
        self.last_decode_batch_ended_at = None
        ...
        self.tracer: Tracer = Tracer(self.id)
        self.hp_timeline_events = []
    ...
    def on_request_arrival(self, seq: Sequence) -> None:
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_ARRIVED_AT
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.absolute_arrived_at,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_IN_WINDOW
        ].put(
            self._get_seq_id(seq.seq_id),
            int(seq.is_in_window),
        )
    ...
    @check_enabled
    def log_hp_event(self, timestamp: float, event: str, count: int) -> None:
        self.hp_timeline_events.append({
            'Instance Id': self.id,
            'Time': timestamp,
            'Event': event,
            'Count': count
    ...
    @check_enabled
    @if_write_metrics
    def on_request_offload(self, seq: Sequence) -> None:
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_OFFLOADED_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            -1,
        )
    ...
    @check_enabled
    @if_write_metrics
    def on_request_offload_arrive(self, seq: Sequence) -> None:
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_OFFLOADED_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.request_offloaded_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_ARRIVED_AT
        ].put(
    ...
    def _on_request_end(self, seq: Sequence) -> None:
        ...
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_FINISHED_AT
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.absolute_finished_at,
        )
        ...
        self.seq_metrics_histogram[SequenceMetricsHistogram.REQUEST_SEQ_PERPLEXITY].put(
            self._get_seq_id(seq.seq_id), seq.state.seq_perplexity
        )
        ...
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_PAUSED_TIME
        ].put(self._get_seq_id(seq.seq_id), seq.state.paused_time)
        self.seq_metrics_time_distributions[
        ...
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.DECODE_TBT_MEAN
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.decode_tbt_mean,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.KV_TRANSFER_NETWORK_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.kv_transfer_network_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.KV_META_IDLE_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.kvmeta_idle_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.KV_META_TRASNFER_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.kvmeta_transfer_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.KV_META_SCHEDULE_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.kvmeta_schedule_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.KV_INSERTION_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.kv_insertion_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.WORKER_KV_INSERTION_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.worker_kv_insertion_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.WORKER_KV_ALLOCATE_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.worker_kv_allocate_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_RESCHEDULE_INTERVAL
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.request_reschedule_interval,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.REQUEST_PREEMPTION_COUNT
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.request_preemption_count,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.ESTIMATED_PREFILL_TIME
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.estimated_prefill_time,
        )
        self.seq_metrics_time_distributions[
            SequenceMetricsTimeDistributions.HIGH_PRIORITY_SCHEDULING_DELAY
        ].put(
            self._get_seq_id(seq.seq_id),
            seq.state.high_priority_scheduling_delay,
        )
    ...
    def _update_per_token_execution_times(
        ...
        # If not true, don't put the data in it because it uses CDFSketch
    ...
    def on_schedule(
        ...
        if scheduler_metadata.batch.is_decoding():
            if self.last_decode_batch_ended_at is not None:
                self.batch_metrics_time_distribution[
                    BatchMetricsTimeDistribution.INTER_DECODE_BATCH_DELAY
                ].put_pair(
                    scheduler_metadata.id,
                    end_time - self.last_decode_batch_ended_at,
                )

        if not self.config.enable_chrome_trace:
            return

        self.tracer.on_schedule(scheduler_metadata, start_time, end_time)
    ...
    def on_batch_stage_end(
        ...
        self.tracer.on_batch_stage_end(scheduler_outputs, start_time, end_time)
    ...
    @check_enabled
    @if_write_metrics
    def on_aborted_requests(self, aborted_seq_list: List[Sequence]):
        for seq in aborted_seq_list:
            self.seq_metrics_time_distributions[
                SequenceMetricsTimeDistributions.REQUEST_ABORTED
            ].put(
                self._get_seq_id(seq.seq_id),
                seq.state.request_aborted,
            )
            self.completion_metrics_time_series[
                CompletionMetricsTimeSeries.REQUEST_COMPLETION
            ].put(seq.state.completed_at, 1)
    ...
    def on_batch_end(
        ...
        if scheduler_metadata.batch.is_decoding():
            self.last_decode_batch_ended_at = batch_end_time

        if self.config.enable_chrome_trace:
            self.tracer.on_batch_end(scheduler_outputs, batch_start_time, batch_end_time)
        ...
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_PREFILL_TOKEN_LIST
        ].put_pair(scheduler_metadata.id, scheduler_metadata.prefill_token_list)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_DECODE_SEQ_ID_LIST
        ].put_pair(scheduler_metadata.id, scheduler_metadata.batch.decode_ids)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_PREFILL_SEQ_ID_LIST
        ].put_pair(scheduler_metadata.id, scheduler_metadata.batch.prefill_ids)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_CHUNK_TOKEN_LIST
        ].put_pair(scheduler_metadata.id, scheduler_metadata.chunk_token_list)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_PROCESSED_TOKEN_LIST
        ].put_pair(scheduler_metadata.id, scheduler_metadata.processed_token_list)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_ESTIMATED_RUNTIME
        ].put_pair(scheduler_metadata.id, scheduler_metadata.predict_batch_time)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_REMAINING_COMPUTE
        ].put_pair(scheduler_metadata.id, scheduler_metadata.remaining_compute)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_REMAINING_TOKENS
        ].put_pair(scheduler_metadata.id, scheduler_metadata.remaining_tokens)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_REMAINING_KV_BLOCKS
        ].put_pair(scheduler_metadata.id, scheduler_metadata.remaining_kv_blocks)
    ...
    def _save_as_csv(
        ...
        for df in dataseries_dfs:            
            assert df[[key_to_join, INSTANCE_ID_STR]].notnull().all(axis=1).all(), \
                f"Null values found in key columns: {key_to_join}, {INSTANCE_ID_STR}"      
            assert not df.duplicated(subset=[key_to_join, INSTANCE_ID_STR]).any(), \
                f"Duplicate key combinations found in DataFrame on columns: {key_to_join}, {INSTANCE_ID_STR}"
        merged_df = reduce(
            lambda left, right: left.merge(right, on=[key_to_join, INSTANCE_ID_STR], how="outer"),
            dataseries_dfs,
        )
    ...
    def _store_completion_metrics(self, base_plot_path: str):
        ...
        if self.config.keep_individual_batch_metrics:
            for dataseries in self.token_metrics_time_list.values():
                ...
                dataseries.plot_cdf(base_plot_path, dataseries.y_name, DECODE_TOKEN_ID_STR)
    ...
    def _store_chrome_trace(self):
        ...
        self.tracer.store_traces(self.output_dir)
    ...
    def _store_hp_timeline(self):
        if not self.hp_timeline_events:
            return
        df = pd.DataFrame(self.hp_timeline_events)
        df.to_csv(f"{self.output_dir}/hp_timeline.csv", index=False)
    ...
    def plot(self):
        ...
        if self.config.write_metrics and getattr(self, "initial_memory_profiling_done", False):
        ...
        if getattr(self.config, "write_metrics", False):
            self._store_hp_timeline()
    ...
    def merge(self, other: "MetricsStore"):
        ...
        self.hp_timeline_events.extend(other.hp_timeline_events)
