...
class BaseSequenceManager(ABC):
    ...
    def __init__(self, config: SystemConfig):
        ...
        self.config = config
        self.use_logits = config.controller_config.use_logits
    ...
    def get_seq(self, id) -> Sequence:
        return self.seq_map[id]

    def _free_seq(self, seq_id: str) -> None:
        assert seq_id in self.seq_map, f"seq manager free seq {seq_id}"
    ...
    def _distserve_preempt_seq(self, seq_id: str) -> None:
        # when swap to DRAM, the worker remove the seq id from the map 
        # "get_kv_cache", so we keep the same logic here
        assert seq_id in self.seq_map
        seq = self.seq_map[seq_id]
        assert seq.is_executing(), f"seq_id: {seq_id}, status: {seq.get_status()}"
        seq.reset_butnot_recompute()
    ...
    def _on_seq_scheduled(self, seq_sched_metadata: SequenceScheduleMetadata) -> None:
        assert seq_sched_metadata.seq_id in self.seq_map, f"seq {seq_sched_metadata.seq_id} not in seq map, {type(seq_sched_metadata.seq_id)}"
    ...
    def on_schedule(
        ...
        for seq_id in scheduler_outputs.preempted_seq_ids:
            if scheduler_outputs.preempt_recompute:
                self._preempt_seq(seq_id)
            else:
                self._distserve_preempt_seq(seq_id)
    ...
    def on_step_completed(
        ...
        for scheduled_seq_metadata, sampler_output in zip(
            ...
            if not seq.prompt_processing_finished:
                if self.use_logits:
                    seq.calculate_seq_perplexity(sampler_output)
