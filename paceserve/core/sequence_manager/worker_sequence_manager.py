...
class WorkerSequenceManager(BaseSequenceManager):
    ...
    def _distserve_preempt_seq(self, seq_id: str) -> None:
        # here the worker frees both the blcok space as well as seq map
        super()._distserve_preempt_seq(seq_id)
        self._free_seq(seq_id)
        #print(f"space on worker after preempt seq {seq_id} : {self.block_manager.get_num_free_gpu_blocks()}")
