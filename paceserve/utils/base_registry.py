...
class BaseRegistry(ABC):
    ...
    @classmethod
    def get_ray_actor(cls, key: Enum, *args, num_cpus=4, max_concurrency=2, **kwargs) -> Any:
        if key not in cls._registry:
            raise ValueError(f"{key} is not registered")
        return cls._registry[key].options(num_cpus=num_cpus, max_concurrency=max_concurrency).remote(*args, **kwargs)
