    ...
    if name.lower() == "sharegpt":
        ...
        for data in tqdm.tqdm(dataset):
            ...
            if prompt_len + output_len >= 2048:
                ...
                continue
            sar_data.append({"num_prefill_tokens": prompt_len, "num_decode_tokens": output_len, "num_total_tokens": prompt_len+output_len, "pd_ratio": prompt_len/output_len})
            result.append(TestRequest(prompt, prompt_len, output_len))
        df = pd.DataFrame(sar_data)
        df.to_csv("sharegpt.csv", index=False)
        ...
        for file in tqdm.tqdm(files):
            with open(file, "r") as f:
                for line in f.readlines():
                    ...
                    context_len_allowed = min(8192 - answer_len, random.randint(args.longbench_min_prompt_len, args.longbench_max_prompt_len))
                    ...
                    sar_data.append({"num_prefill_tokens": len(context_token_ids), "num_decode_tokens": answer_len, "num_total_tokens": len(context_token_ids)+answer_len, "pd_ratio": len(context_token_ids)/answer_len})
        df = pd.DataFrame(sar_data)
        df.to_csv("long_bench.csv", index=False)
        ...
        with open(dataset_path, "r") as f:
            for line in f.readlines():
                ...
                sar_data.append({"num_prefill_tokens": len(context_token_ids), "num_decode_tokens": len(answer_token_ids), "num_total_tokens": len(context_token_ids)+len(answer_token_ids), "pd_ratio": len(context_token_ids)/len(answer_token_ids)})
        df = pd.DataFrame(sar_data)
